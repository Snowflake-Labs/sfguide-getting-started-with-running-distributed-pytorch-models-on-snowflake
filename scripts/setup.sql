...
-- Create a compute pool for producers
ALTER COMPUTE POOL IF EXISTS TASTYBYTESENDTOENDML_PRODUCER_POOL STOP ALL;
DROP COMPUTE POOL IF EXISTS TASTYBYTESENDTOENDML_PRODUCER_POOL;
CREATE COMPUTE POOL TASTYBYTESENDTOENDML_PRODUCER_POOL 
    MIN_NODES = 1, 
    MAX_NODES = 1, 
    INSTANCE_FAMILY = CPU_X64_XS;

-- Grant all privileges on the producer compute pool to the SYSADMIN role
GRANT ALL PRIVILEGES ON COMPUTE POOL TASTYBYTESENDTOENDML_PRODUCER_POOL TO ROLE SYSADMIN;
...
-- Create a compute pool for consumers
ALTER COMPUTE POOL IF EXISTS TASTYBYTESENDTOENDML_CONSUMER_POOL STOP ALL;
DROP COMPUTE POOL IF EXISTS TASTYBYTESENDTOENDML_CONSUMER_POOL;
CREATE COMPUTE POOL TASTYBYTESENDTOENDML_CONSUMER_POOL 
    MIN_NODES = 1, 
    MAX_NODES = 5, 
    INSTANCE_FAMILY = GPU_NV_S;

-- Grant all privileges on the consumer compute pool to the data scientist role
GRANT ALL PRIVILEGES ON COMPUTE POOL TASTYBYTESENDTOENDML_CONSUMER_POOL TO ROLE TASTYBYTESENDTOENDML_DATA_SCIENTIST;
